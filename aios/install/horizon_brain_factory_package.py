#!/usr/bin/env python3
"""horizon_brain_factory_package.py — install / uninstall / update / status for the
Horizon Brain Factory package.

This is a SEPARATE package from the Horizon AIOS core. Unlike LAPP (which file-drops a skill), the
brain factory is a heavy CLI deployment toolchain, so `install` does NOT copy the toolchain anywhere:
...
uses the package-scoped horizon_brain_factory_* name (it is not part of the OS core).
...
Source model: the DEVELOPMENT CANON is the push-enabled checkout (e.g. projects/horizon_brain_factory),
...
def package_root() -> Path:
    # this file is <pkg>/aios/install/horizon_brain_factory_package.py
...
def cmd_install(args) -> None:
    ...
    print(f"Done. The factory is registered and discoverable. Deploy a brain by running the clone's\n"
...
# --------------------------------------------------------------------------- main
def main() -> None:
    ap = argparse.ArgumentParser(description="Install/uninstall/update/status the Horizon Brain Factory package.")

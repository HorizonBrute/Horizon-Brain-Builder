...
rollback() {
  ...
  for rec in "${TOUCHED[@]}"; do
    dst="${rec%%$'\t'*}"; bak="${rec#*$'\t'}"
    # rm the dest first: a synced file may be mode 0555 (read-only even to its owner), which
    # makes a plain `cp -p` onto it fail ("Permission denied"). Removing it (the dir is
    # writable) then copying the backup fresh restores it regardless of the old file's mode.
    if [ "$bak" = "NEW" ]; then rm -f "$dst"; else rm -f "$dst"; cp -p "$bak" "$dst"; fi
  done
}
